...
from src.models.request_models import SenderRequest, SenderUpdateRequest
...
def test_update_sender_patch_updates_name(sender_db):
    alice = sender_db.exec(select(Sender).where(Sender.name == "Alice")).first()
    service = SenderService(session=sender_db)
    result = service.update_sender_patch(
        sender_id=alice.id,
        sender_request=SenderUpdateRequest(name="Alicia"),
    )

    assert result.name == "Alicia"


def test_update_sender_patch_updates_email(sender_db):
    alice = sender_db.exec(select(Sender).where(Sender.name == "Alice")).first()
    service = SenderService(session=sender_db)
    result = service.update_sender_patch(
        sender_id=alice.id,
        sender_request=SenderUpdateRequest(email="alicia_new@example.com"),
    )

    assert result.email == "alicia_new@example.com"


def test_update_sender_patch_updates_address(sender_db):
    alice = sender_db.exec(select(Sender).where(Sender.name == "Alice")).first()
    service = SenderService(session=sender_db)
    result = service.update_sender_patch(
        sender_id=alice.id,
        sender_request=SenderUpdateRequest(address="99 New Road"),
    )

    assert result.address == "99 New Road"


def test_update_sender_patch_updates_contact_no(sender_db):
    alice = sender_db.exec(select(Sender).where(Sender.name == "Alice")).first()
    service = SenderService(session=sender_db)
    result = service.update_sender_patch(
        sender_id=alice.id,
        sender_request=SenderUpdateRequest(contact_no="0779999999"),
    )

    assert result.contact_no == "0779999999"


def test_update_sender_patch_does_not_overwrite_unset_fields(sender_db):
    """Fields not included in the PATCH request must remain unchanged."""
    alice = sender_db.exec(select(Sender).where(Sender.name == "Alice")).first()
    service = SenderService(session=sender_db)
    result = service.update_sender_patch(
        sender_id=alice.id,
    ...
    # email and address should be untouched
    assert result.email == "alice@example.com"
    assert result.address == "1 Alpha St"


def test_update_sender_patch_updates_multiple_fields(sender_db):
    alice = sender_db.exec(select(Sender).where(Sender.name == "Alice")).first()
    service = SenderService(session=sender_db)
    result = service.update_sender_patch(
        sender_id=alice.id,
        sender_request=SenderUpdateRequest(name="Alicia", address="77 New Lane"),
    )

    assert result.name == "Alicia"
    assert result.address == "77 New Lane"


def test_update_sender_patch_persists_to_db(sender_db):
    alice = sender_db.exec(select(Sender).where(Sender.name == "Alice")).first()
    service = SenderService(session=sender_db)
    service.update_sender_patch(
        sender_id=alice.id,
        sender_request=SenderUpdateRequest(name="Alicia"),
    )
...
def test_update_sender_patch_returns_sender_response(sender_db):
    alice = sender_db.exec(select(Sender).where(Sender.name == "Alice")).first()
    service = SenderService(session=sender_db)
    result = service.update_sender_patch(
        sender_id=alice.id,
...
def test_update_sender_patch_raises_not_found_for_unknown_id(sender_db):
    service = SenderService(session=sender_db)

    with pytest.raises(NotFoundException):
        service.update_sender_patch(
            sender_id=uuid.uuid4(),
            sender_request=SenderUpdateRequest(name="Ghost"),
        )


def test_update_sender_patch_raises_conflict_on_duplicate_email(monkeypatch, sender_db):
    alice = sender_db.exec(select(Sender).where(Sender.name == "Alice")).first()
    service = SenderService(session=sender_db)
    monkeypatch.setattr(
        sender_db, "commit",
    ...
    with pytest.raises(ConflictException) as exc_info:
        service.update_sender_patch(
            sender_id=alice.id,
            sender_request=SenderUpdateRequest(email="carol@example.com"),
        )
    assert "Email" in exc_info.value.message


def test_update_sender_patch_raises_conflict_on_duplicate_contact_no(monkeypatch, sender_db):
    alice = sender_db.exec(select(Sender).where(Sender.name == "Alice")).first()
    service = SenderService(session=sender_db)
    monkeypatch.setattr(
        sender_db, "commit",
    ...
    with pytest.raises(ConflictException) as exc_info:
        service.update_sender_patch(
            sender_id=alice.id,
            sender_request=SenderUpdateRequest(contact_no="0772222222"),
        )
    assert "Contact" in exc_info.value.message


def test_update_sender_patch_raises_conflict_on_unknown_constraint(monkeypatch, sender_db):
    alice = sender_db.exec(select(Sender).where(Sender.name == "Alice")).first()
    service = SenderService(session=sender_db)
    monkeypatch.setattr(
        sender_db, "commit",
    ...
    with pytest.raises(ConflictException) as exc_info:
        service.update_sender_patch(
            sender_id=alice.id,
            sender_request=SenderUpdateRequest(name="X"),
        )
    assert "Duplicate values violates unique constraint" in str(exc_info.value)


def test_update_sender_patch_rolls_back_on_integrity_error(monkeypatch, sender_db):
    alice = sender_db.exec(select(Sender).where(Sender.name == "Alice")).first()
    service = SenderService(session=sender_db)
    rollback_mock = MagicMock()
    monkeypatch.setattr(
...
def test_update_sender_patch_raises_internal_on_db_error(monkeypatch, sender_db):
    alice = sender_db.exec(select(Sender).where(Sender.name == "Alice")).first()
    service = SenderService(session=sender_db)
    monkeypatch.setattr(
        sender_db, "commit",
...
def test_update_sender_patch_rolls_back_on_db_error(monkeypatch, sender_db):
    alice = sender_db.exec(select(Sender).where(Sender.name == "Alice")).first()
    service = SenderService(session=sender_db)
    rollback_mock = MagicMock()
